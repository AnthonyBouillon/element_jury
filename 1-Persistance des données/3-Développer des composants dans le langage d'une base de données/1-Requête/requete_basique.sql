...
SELECT commande_produit.id_produit AS "Référence du produit", description_court_produit AS "Nom du produit", quantite AS "Quantité commandé"  
...
SELECT date_commande AS "Date de la commande", `client`.id_client AS "Référence client", prix_total * quantite AS "Montant" FROM `client` 
...
SELECT statut_client, nom_client AS "Nom du client", SUM((prix_total * quantite) - coeff_client/100) AS "Chiffre d'affaire" FROM CLIENT 
JOIN commande ON CLIENT.id_client = commande.id_client 
JOIN commande_produit ON commande.id_commande = commande_produit.id_commande 
GROUP BY nom_client;

-- 5 - Lister les commandes en cours de livraison
SELECT commande.etat_commande, commande.id_commande AS "Référence de la commande" 
FROM commande  
WHERE etat_commande = "En cours de livraison";